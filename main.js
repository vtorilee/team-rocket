...
function drawStartup(group) {
  ...
  function drawSpeechText() {
    const textGroup = speechBubble.append("g").attr("class", "dialogue-layer");
    let currentLine = 0;

    //y-pos offsets for each text line
    const yOffsets = [53, 177, 290];
    //x-offsets for button
    const xOffsets = [390, 470, 420];
  ...
  const teamIcons = group.append("g").attr("transform", `translate(180, 175)`);
  teamIcons
    .append("image")
...
function drawIntro(group) {
  //meowth section
  const meowthGroup = group
    .append("g")
    .attr("transform", "translate(-385, -115)");

  const meowth = meowthGroup
    .append("image")
  ...
  const speechBubble = meowthGroup
    .append("image")
  ...
  const textLayer = meowthGroup.append("g").attr("class", "meowth-intro-layer");
  ...
  function displayStartingButtons() {
    const buttonGroup = meowthGroup
      .append("g")
      .attr("transform", "translate(500, 375)")
      .style("opacity", 0);
